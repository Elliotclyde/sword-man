...
class MainScene extends Phaser.Scene {
  ...
  initializeLevel() {
    ...
    if (this.wallDecorations) {
      const wallDecorationsToDestroy = [
        ...this.wallDecorations.children.entries,
      ];
    ...
    if (this.peninsulaSprites) {
      const peninsulaSpritesToDestroy = [
        ...this.peninsulaSprites.children.entries,
      ];
    ...
    this.playerPotionCollider = this.physics.add.overlap(
      this.player,
      this.potions,
    ...
    this.enemyWallCollider = this.physics.add.collider(
      this.enemies,
      this.walls,
    ...
    this.playerPeninsulaWallCollider = this.physics.add.collider(
      this.player,
      this.peninsulaWalls,
    );
    ...
    this.enemyPeninsulaWallCollider = this.physics.add.collider(
      this.enemies,
      this.peninsulaWalls,
      this.handleEnemyWallCollision,
      null,
    ...
    if (this.currentLevelIndex === 0 && !this.audioManager) {
    ...
    if (this.audioManager) {
      const currentLevel = this.levels[this.currentLevelIndex];
      const musicKey = currentLevel.customMusic || "main";
  ...
  create() {
    ...
    this.wallGenerationParams = {
      tileSize,
      gridWidth,
      playableGridHeight: PLAYABLE_GRID_HEIGHT,
      scale,
    };
    ...
    this.anims.create({
      key: "werewolf_walk",
      frames: this.anims.generateFrameNumbers("werewolf", {
        start: 13,
        end: 20,
      }),
      frameRate: 10,
  ...
  spawnDoor(tileSize, gridWidth, scale) {
    ...
    if (doorGridX > 0) {
      ...
      const leftWallTile = this.add.sprite(
        leftWallWorldX,
        doorWorldY,
        "dungeon",
        leftWallFrame,
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const doorTile = this.add.sprite(
        doorWorldX,
        doorWorldY,
        "dungeon",
        doorFrames[i],
    ...
    if (doorGridX + 2 < gridWidth) {
      ...
      const rightWallTile = this.add.sprite(
        rightWallWorldX,
        doorWorldY,
        "dungeon",
        rightWallFrame,
  ...
  spawnWalls(tileSize, gridWidth, playableGridHeight, scale) {
    ...
    const rightWall = this.add.sprite(
      rightX,
      600 - scaledTileSize,
      "dungeon",
      45,
  ...
  generatePeninsulas(
    ...
    const playerSpawnGridX = levelConfig
      ? Math.floor(levelConfig.playerStartX / scaledTileSize)
      : -1;
    const playerSpawnGridY = levelConfig
      ? Math.floor(levelConfig.playerStartY / scaledTileSize)
      : -1;

    for (let p = 0; p < peninsulaCount; p++) {
      let peninsulaCreated = false;
      const maxAttempts = 3;
      console.log(`\n=== Attempting to create peninsula ${p + 1} ===`);

      // Attempt to create peninsula up to 3 times
      for (
        let attempt = 0;
        attempt < maxAttempts && !peninsulaCreated;
        attempt++
      ) {
        ...
        // Check if peninsula overlaps with already-placed peninsulas (must have at least 2 empty tiles between them)
        let overlapsWithOtherPeninsula = false;
        const minGap = 2;
        console.log(
          `Checking overlap for attempt ${attempt + 1}: side=${side}, gridX=${gridX}, gridY=${gridY}, width=${peninsulaWidth}, height=${peninsulaHeight}`,
        );
        console.log(`Current peninsulas in array: ${this.peninsulas.length}`);
        for (const existingPeninsula of this.peninsulas) {
          // Check if rectangles are separated by at least minGap in BOTH axes
          // Rectangles are separated if one is completely to the side/above the other with a gap
          const separatedByX =
            gridX + peninsulaWidth + minGap <= existingPeninsula.gridX ||
            existingPeninsula.gridX + existingPeninsula.gridWidth + minGap <=
              gridX;
          const separatedByY =
            gridY + peninsulaHeight + minGap <= existingPeninsula.gridY ||
            existingPeninsula.gridY + existingPeninsula.gridHeight + minGap <=
              gridY;

          console.log(
            `  vs existing: gridX=${existingPeninsula.gridX}, gridY=${existingPeninsula.gridY}, width=${existingPeninsula.gridWidth}, height=${existingPeninsula.gridHeight}`,
          );
          console.log(
            `    separatedByX=${separatedByX} (new ends at ${gridX + peninsulaWidth}, existing starts at ${existingPeninsula.gridX}; or existing ends at ${existingPeninsula.gridX + existingPeninsula.gridWidth}, new starts at ${gridX})`,
          );
          console.log(
            `    separatedByY=${separatedByY} (new ends at ${gridY + peninsulaHeight}, existing starts at ${existingPeninsula.gridY}; or existing ends at ${existingPeninsula.gridY + existingPeninsula.gridHeight}, new starts at ${gridY})`,
          );

          // If NOT separated in either axis, they overlap
          if (!separatedByX || !separatedByY) {
            console.log(`    OVERLAP DETECTED! Rejecting this placement.`);
            overlapsWithOtherPeninsula = true;
            break;
          } else {
            console.log(`    OK - separated properly`);
          }
        }
        ...
        // Add to peninsulas array immediately so subsequent peninsulas see it in overlap checks
        console.log(
          `PUSHING to array. Array before: ${this.peninsulas.length} peninsulas`,
        );
        this.peninsulas.push(peninsula);
        console.log(
          `Peninsula ${this.peninsulas.length} created: side=${side}, gridX=${gridX}, gridY=${gridY}, width=${peninsulaWidth}, height=${peninsulaHeight}`,
        );
        console.log(`Array after push: ${this.peninsulas.length} peninsulas`);
        ...
        for (let ty = 0; ty < peninsulaHeight; ty++) {
          for (let tx = 0; tx < peninsulaWidth; tx++) {
            ...
            if (isTopLeftCorner) {
              ...
              const cornerSprite = this.add.sprite(
                pixelX,
                pixelY,
                "dungeon",
                cornerFrame,
            ...
            const wall = this.add.zone(
              pixelX + scaledTileSize / 2,
              pixelY + scaledTileSize / 2,
              scaledTileSize,
              scaledTileSize,
            );
  ...
  update() {
    ...
    // Update audio manager for music filter and reverb automation
    if (this.audioManager) {
    ...
    this.enemies.children.entries.forEach((enemy) => {
      ...
      if (!enemy.isDead) {
        if (enemy.isMoving) {
          ...
          enemy.body.setVelocity(
            enemy.direction.x * speed,
            enemy.direction.y * speed,
          );
        } else {
          enemy.body.setVelocity(0, 0);
        }
      ...
      if (
        enemy.type === enemyTypes.WEREWOLF &&
        !enemy.isDead &&
        !enemy.isAxeSwinging
      ) {
      ...
      if (!enemy.isDead && !enemy.isAxeSwinging) {
        ...
        if (
          ...
          if (enemy.isMoving) {
            ...
            if (
              !enemy.anims.isPlaying ||
              enemy.anims.currentAnim.key !== standAnimKey
            ) {
      ...
      if (timeSinceLastEdgeHit > 50) {
        if (enemy.x + enemyBodyOffsetX - enemyWidth < SCREEN_BUFFER) {
          ...
          enemy.x + enemyBodyOffsetX + enemyWidth >
          800 - SCREEN_BUFFER
        ) {
        ...
        if (enemy.y + enemyBodyOffsetY - enemyHeight < SCREEN_BUFFER) {
          ...
          enemy.y + enemyBodyOffsetY + enemyHeight >
          600 - SCREEN_BUFFER
        ) {
  ...
  createHealthBarBloodParticles() {
    ...
    for (let i = 0; i < count; i++) {
      const initialX = Phaser.Math.Between(
  ...
  emitDashParticles(playerX, playerY, dashAngle) {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const angleVariation = Phaser.Math.FloatBetween(
        -Math.PI / 6,
        Math.PI / 6,
      );
  ...
  handlePlayerEnemyCollision(player, enemy) {
    ...
    if (this.isPlayerSwinging && !enemy.isDead) {
      ...
      if (
        (playerFacingLeft && !enemyIsToTheLeft) ||
        (!playerFacingLeft && enemyIsToTheLeft)
      ) {
      ...
      if (
        !enemy.isDead &&
        !enemy.hasRecentlyAttacked &&
        enemyConfig.canDamageOnTouch
      ) {
        // Check if player is currently invulnerable to damage
        if (
          this.playerLastHitTime &&
  ...
  checkAndTurnEnemyAround(enemy) {
    ...
    if (playerCurrentlySide === enemy.detectionSideOnCollision) {
      ...
      enemy.lastHorizontalDirection =
        enemy.lastHorizontalDirection === "left" ? "right" : "left";
    }
